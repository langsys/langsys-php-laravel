...
| **Core bound against** | langsys-php-sdk `fdbf84e` (`v1.3.1-44`, `feature/838_write_key_gating_reland`, untagged), whose conformance file is canonical — see *Release gate* |
| **Derived** | 2026-09-13T00:57:30Z by `git rev-parse` on the commit above; the text graded is byte-identical to the text read |
...
| REG-10 | delegated | - | Core `REG-10`: **partial** — no throw into a render and no success-shaped result are met; *always log* is not, because of the `NullLogger` default this file raised. Counts red here until the core's row turns green. The binding adds no failure behaviour of its own. It never throws into a render from the write lane, `RequestScopeTest::testAFlushThatCannotReachTheApiFailsNothing`. Its flush call sites return nothing, success-shaped or otherwise. It swallows nothing the core lets through: `LangsysTranslatorTest::testDoesNotSwallowAFailureTheSdkLetThrough` and `TranslateResponseTest::testDoesNotSwallowAFailureTheSdkLetThrough`, M7b, M9. The *always log* half is the core's logger, which defaults to a `NullLogger` — raised below. |
...
| CID-3 | delegated | - | Core `CID-3`: **partial** — emission is not proven at the core's two public block creators. Counts red here until the core's row turns green. The binding reads content blocks only through `translatePage()`, which carries the core's tolerance, and resolves none itself: `BindingBoundaryTest::testTheBindingReimplementsNoIdentityOrRenderingBehaviour`. |
| CID-4 | delegated | - | Core `CID-4`: **partial** — the legacy-match guard is not proven on the page path, which `TranslateResponse` uses. Counts red here until the core's row turns green. Otherwise as CID-3. |
| TOK-1 | delegated | - | Core `TOK-1`: **partial** — the exclusions are not proven on the page path, which `TranslateResponse` uses. Counts red here until the core's row turns green. The binding tokenizes nothing: `BindingBoundaryTest::testTheBindingReimplementsNoIdentityOrRenderingBehaviour` forbids `DOMDocument`, `DOMXPath`, `HtmlParser`, `MarkupTokenizer` and `preg_replace`. It exposes two routes to the tokenizer and both are the core's: `translate()` for a phrase, and `translatePage()` for markup, handed the exact response bytes, `TranslateResponseTest::testTranslatesAnHtmlResponse`. `TranslateResponseSafetyTest` runs the real page translator over a Laravel-shaped page: script and style bodies survive byte for byte, and nothing from them is queued. |
| TOK-2 | delegated | - | Core `TOK-2`: **partial** — the collapse set is not proven on the page path. Counts red here until the core's row turns green. Otherwise the same evidence as TOK-1. |
| TOK-3 | delegated | - | Core `TOK-3`: **partial** — a measured page-path gap: some attribute phrases register nothing through `translatePage()`. Counts red here until the core's row turns green. Otherwise the same evidence as TOK-1. |
| TOK-4 | delegated | - | Core `TOK-4`: **partial** — the same measured page-path attribute gap as TOK-3. Counts red here until the core's row turns green. Otherwise the same evidence as TOK-1. |
...
| MARK-1 | delegated | - | Core `MARK-1`: **partial** — `translatePage()` serves a resolved block without its identity stamp. Counts red here until the core's row turns green. The binding stamps nothing: `BindingBoundaryTest::testTheBindingReimplementsNoIdentityOrRenderingBehaviour` forbids `data-ls-` and `data-langsys-` in code. |
| MARK-2 | delegated | - | Core `MARK-2`: **partial** — the declared-block host is not proven on the page path. Counts red here until the core's row turns green. The binding reads no host marker: same scan as MARK-1. |
...
| SRV-2 | implemented | n/a (pure) | **Reviewer finding 1, closed.** PHP-FPM builds a container per request; Octane and queue workers do not, so the provider resets the core's per-request state at each boundary. `RequestScopeTest::testTheNextUnitOfWorkReadsTheCatalogAfresh` reads a catalog, moves the shared cache on, crosses each of the three boundaries and asserts the next unit reads the new catalog; M1, M4 and M5 each redden it. **The rule's interleave test is unreachable in this runtime model.** A PHP-FPM process and an Octane worker each serve one request at a time, so concurrent requests never share a `Client`. The reachable failure is sequential reuse across a boundary, which GATE-3 names for exactly these runtimes. It would become live with in-worker concurrency sharing the singleton, such as Swoole coroutine hooks. The catalog memo itself is the core's, core `SRV-2`: **partial** — the core's own suite does not pin `resetRequestState()` clearing that memo, which `::testTheNextUnitOfWorkReadsTheCatalogAfresh` here does, on the real core. Measured: deleting the memo clear from `resetRequestState()` in an archive of core `fdbf84e` reddens it, and the unmutated archive stays green. |
...
Ranked by what each gap costs someone running the Laravel stack, not by rule order. Items 1 to 7 are the core's to close; this package delegates them by rule.

1. **REG-8 — no backoff, and long-lived workers amplify it.** The core retains a failed send, and `resetRequestState()` does not clear the retained queue. Under Octane or a queue worker, a failing registration endpoint therefore gets a POST at every request or job, with a payload that grows as new misses join it — reproduced by the core at `fdbf84e`, where REG-8 is held for the operator's clarification on backoff in per-request servers. Load, exactly on the busiest workers.
2. **Automatic mode inherits the core's page-path gaps (TOK-1 to TOK-4, MARK-1, MARK-2, CID-4).** `TranslateResponse` routes every page through `translatePage()`, where the core has measured attribute phrases that register nothing and resolved blocks served without their identity. Tagged mode, through `t()`, does not take that route. Coverage and identity, silently, on every automatic-mode page.
3. **The core logs to a `NullLogger` by default (REG-10, and REG-3 per the spec's Open section).** In a default Laravel app, a degraded lookup, a dropped flush and a failed send are all recorded nowhere. Diagnosability. Routing the core's logger to a Laravel channel is deferred in `ROADMAP.md`, because it changes log volume.
4. **OBS-1 — a misconfigured key is completely silent.** Nothing is queued, nothing errors, nothing logs.
5. **REG-11 — no ellipsis diagnostic.** Catalog pollution and double translation spend.
6. **WIRE-3 in the core's entry points.** This package compensates, but `Langsys::client()->translate('Save', 'es-ES')` — the documented escape hatch — splits the catalog cache and fetches twice.
7. **ICU-1 and CID-3 are proven inside the core but not on its public routes.** Nothing in this package reaches around those routes, so the exposure is the core's proof, not a Laravel path.
8. **CONF-1 / CONF-2 — the shared contract fixture.** Until it exists, SRV-1, SRV-3 and CONF-1 stay `provisional`.
...
**Re-run against a `git archive` of core `fdbf84e`, 2026-09-13T00:55Z.** The PHP lane disclosed that its own mutation batteries rewrote its `src/` in place until about 00:44Z, and the first run of this record read that tree through the path repository inside that window. The re-run used an archive no one could touch: all 25 mutants reddened the same tests, plus `testAFlushThatCannotReachTheApiFailsNothing` under M1, a test added after M1's first run. The table shows the re-run.

| Mutant | What it breaks | Reddened |
|---|---|---|
| M1 | boundary never calls `resetRequestState()` | `RequestScopeTest`: `testEveryBoundaryFlushesAndThenResets`, `testTheNextUnitOfWorkReadsTheCatalogAfresh`, `testTheNextUnitOfWorkStartsWithoutAWriteDecision`, `testAFlushThatCannotReachTheApiFailsNothing` |
...
- **Core WIRE-3: `Client::translate()` and `getTranslations()` do not normalize the locale they are handed.** Acknowledged: the core reproduced it and grades WIRE-3 `partial` at `fdbf84e`, where it is still unfixed (re-measured). Reproduction: cache an `es-es` catalog, call `translate('Save', 'es-ES')`. The core reads `translations_<project>_es-ES`, misses, and goes to the network. Only `setLocale()` normalizes.
- **Core HINT-2 was mis-graded — resolved.** Regraded `implemented` at `fdbf84e`. It is rowed `n/a (profile: server)` inside the `HINT-1 … HINT-8` range, but HINT-2's profile is `server`. Non-participation is testable, as it is for GRANT's `X-Write-Grant` clause.
- **Core REG-10: the default `NullLogger`.** Acknowledged: `partial` at `fdbf84e`, with the logging half unmet. "Always log" is unmet in any integration that does not enable core logging, and that includes a default Laravel app.
...
as graded in this file
  ...
  not implemented                 0
  held (strip ruling)             0
delegated rows resolved against langsys-php-sdk fdbf84e
  implemented                     8
  not implemented                 3
  partial                         10
  provisional                     16
counting red                      REG-8, REG-10, REG-11, ICU-1, CID-3, CID-4, TOK-1, TOK-2, TOK-3, TOK-4, MARK-1, MARK-2, OBS-1
