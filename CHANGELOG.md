...
- **`translate="no"` is the documented exclusion marker, deliberately.** The SDK also accepts `data-notrans`, but in the shipped `langsys/langsys-php` v1.1.0 its semantics are inverted — verified here: bare `data-notrans` is **registered into the shared catalog**, while both `="true"` and `="false"` exclude. Upstream has fixed it, but no release carries the fix yet, so this package points only at `translate="no"`, which is unaffected.
