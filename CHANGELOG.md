...
### Fixed upstream (requires `langsys/langsys-php` ^1.3)

- **Automatic mode now flushes after the response, like tagged mode.** Reported from here: `translatePage()` bypassed the pending-registration queue, calling `canWrite()`, `registerPhrases()` (one POST *per content block*, in a loop), then `clearCache()` + `getTranslations()` to refetch — all inline mid-render, so `FlushPendingRegistrations` and the Octane listener governed `t()`/`@t` but not automatic mode. Upstream confirmed it was a coupling rather than a constraint: the refetch could only surface translations registered by an *earlier* request, which the next page load picks up anyway. v1.3.0 routes page registration through the same queue, taking a page with four new blocks and four new phrases from 5 POSTs + 1 GET during render to **zero**, with 2 batched POSTs at flush. The development-only caveat on write keys is withdrawn.
  - Behaviour change: a page no longer picks up translations an earlier request registered *within the same response*; they appear on the next one.
...
### Changed (adopting `langsys/langsys-php` v1.3.0)

- **Pinned `^1.3`** for the queued page registration above. `TranslateResponseSafetyTest` now captures at `Client::getPendingPhrases()` rather than overriding `registerPhrases()` — the boundary that actually decides what reaches the catalog, and one that fails loudly rather than silently if it moves again. Every absence assertion is paired with a positive control proving the capture point is live.
