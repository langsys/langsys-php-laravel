...
- **Inertia (MSG-12).** When `inertiajs/inertia-laravel` is installed, the middleware shares the flashed entries as a page prop under the same key on the way *in*, before the page renders, so the destination page's JS SDK can render them through MSG-5. Inertia's own conditional props are no help here — `lazy` and `optional` withhold a prop from exactly the full page load this has to reach — so the sharing is conditional instead, and a page that follows no failure carries no prop of ours. Inertia's own `errors` prop is untouched, and carries source text. Inertia is a dev dependency of this package only: nothing reaches an application that does not already use it.
...
- **The marker and the gate have landed in the TypeScript core** (`793596f2`): `data-ls-resolved` is inherited with the nearest ancestor deciding, `data-langsys-resolved` is accepted on read, `="false"` opts a subtree out, and the value is informational — so a bare attribute is enough. The registration gate, `discovery_base_locale_only`, arrives in the authorize response as well as the catalog envelope, so a request that only validates never needs a catalog fetch to learn it. **This package's consumer side — a Blade directive printing the attribute, and a helper for the JSON manifest — waits for Langsys to spec the clauses**, because the spec wins over any SDK's tests.
